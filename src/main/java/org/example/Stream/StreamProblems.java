...
//        System.out.println("\n\nExercise 3 — Obtain a list of product with category = “Toys” and then apply 10% discount");
      products.stream().filter(p->p.getCategory().equals("Toys")).map(p->{
       ...
       double sum1 = orders.stream().filter(o->o.getOrderDate().getMonthValue() == 2 )
        ...
        System.out.println("\n\nExercise 11 — Obtain a data map with order id and order’s product count");
        System.out.println(orders.stream().collect(Collectors.toMap(Order::getId, o-> (long) o.getProducts().size())));
        ...
        System.out.println("\n\nExercise 14 — Obtain a data map with list of product name by category");
          products.stream().collect(Collectors.groupingBy(Product::getCategory)).forEach((cat , names) -> System.out.println(cat +" : " + names.stream().map(Product::getName).collect(Collectors.joining(","))));
